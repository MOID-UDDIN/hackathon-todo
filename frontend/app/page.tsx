...
export default function LoginPage() {
  ...
  // original err + extra friendly message
  ...
  const topBanner = useMemo(() => {
    return "✅ Welcome! This is a Todo App — login to manage your tasks.";
  }, []);

  function setNiceErrors(raw: any) {
    const msg = (raw?.message ?? raw ?? "").toString();

    // keep original error too (for debugging / transparency)
    setErr(msg || "Login failed");

    const lower = msg.toLowerCase();

    // Friendly, non-breaking hints (doesn't change backend behavior)
    if (
      lower.includes("user not found") ||
      lower.includes("not found") ||
      lower.includes("no user") ||
      lower.includes("does not exist") ||
      lower.includes("account") && lower.includes("exist") ||
      lower.includes("404")
    ) {
      setHint("Account doesn’t exist. Please sign up first.");
      return;
    }
    ...
    if (
      ...
      setHint("Incorrect email or password. Please try again.");
      return;
    }

    // fallback generic hint
    setHint("Couldn’t log you in. Please check your details and try again.");
  }
    ...
    <main className="min-h-screen flex items-center justify-center bg-gray-100 px-4 py-6">
      <div className="w-full max-w-sm bg-white rounded-xl shadow-md p-5 sm:p-6">
        ...
        <form onSubmit={onSubmit} className="space-y-4">
          ...
          {hint && (
            <p className="text-sm text-center rounded-lg bg-yellow-50 px-3 py-2 text-yellow-800">
              {hint}{" "}
              {hint.toLowerCase().includes("sign up") && (
